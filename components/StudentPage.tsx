...
const NcheIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5 8.281V13.5a1 1 0 001 1h8a1 1 0 001-1V8.281l2.394-1.36a1 1 0 000-1.84l-7-3zM6 9.319l4 2.286 4-2.286V13.5H6V9.319z" /><path d="M6 13.5V15l4 2.286L14 15v-1.5H6z" /></svg>);
...
const StudentResultsView: React.FC<StudentResultsViewProps> = ({ user, onDownload, isDownloading }) => {
    return <div>Student Results Placeholder</div>;
};
...
export const StudentPage: React.FC<StudentPageProps> = ({ user, onLogout }) => {
    ...
    const getModuleIcon = (moduleName: string) => {
        switch(moduleName) {
            ...
            case E_VOTE_MODULE_NAME: return <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a1 1 0 001 1h12a1 1 0 100-2H5V3a1 1 0 00-2 0zm11.707 1.293a1 1 0 00-1.414 0L9 8.586 7.707 7.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l5-5a1 1 0 000-1.414z" clipRule="evenodd" /></svg>;
        ...
        <div className="flex h-screen bg-gray-900 text-white font-sans">
            ...
            {isMobileMenuOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-75 z-40 sm:hidden" onClick={() => setIsMobileMenuOpen(false)}>
                    <aside className="fixed top-0 left-0 h-full w-64 bg-gray-800 shadow-xl z-50 p-4 animate-slide-in-left-fade" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-md hover:bg-gray-700 flex-shrink-0"><CloseIcon /></button>
                        </div>
                        <nav className="space-y-2 flex-grow overflow-y-auto">
                            ...
                            {availableModules.length > 0 && (
                                <div className="pt-4 mt-4 border-t border-gray-700">
                                    <h3 className="px-3 text-xs font-semibold uppercase text-gray-400 tracking-wider mb-2">Modules</h3>
            ...
            <aside className={`bg-gray-800 text-white ${isSidebarExpanded ? 'w-64 p-4' : 'w-20 p-2'} transition-all duration-300 flex-shrink-0 flex-col hidden sm:flex`}>
            ...
            <div className="flex-1 flex flex-col min-h-0">
                <header className="flex-shrink-0 flex items-center justify-between sm:justify-end p-4 bg-gray-800 border-l border-gray-700 shadow-md">
                        <button onClick={() => setIsMobileMenuOpen(true)} className="sm:hidden p-2 -ml-2 text-gray-400 hover:text-white">
                            <HamburgerIcon />
                        </button>
                ...
                <main className={`flex-1 overflow-y-auto border-l border-gray-700 ${!activeModule || activeModule.name !== ONLINE_MODULE_NAME ? 'p-4 lg:p-8' : ''}`}>
                    <div className={`${!activeModule || activeModule.name !== ONLINE_MODULE_NAME ? 'container mx-auto' : 'h-full'}`}>
                        {renderMainContent()}
                    </div>
                </main>
            </div>
